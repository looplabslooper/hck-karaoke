...
## Comandos
- `pnpm dev` — levanta `server` (:8080) + `admin` (:5174) juntos, cada uno con su propio dev server (Vite hace proxy de `/api` y `/library` hacia :8080).
...
- Tras varios reinicios seguidos del servidor durante desarrollo, pueden quedar procesos `node.exe` viejos colgados en los puertos 8080/5174 (`tsx watch` no siempre libera el puerto limpio). Si algo "no responde" o "se cuelga", verificar primero con `netstat -ano | grep LISTENING` y matar duplicados antes de asumir que hay un bug de código nuevo.
- **En dev, la UI vive en `:5174`, no en `:8080`.** El server servía `apps/admin/dist` incondicionalmente, así que entrar a `:8080` mientras se desarrolla te daba **en silencio** un build viejo — parecía andar pero era código de días atrás hablando con una API que ya había cambiado (síntoma real: `TypeError: v.find is not a function`, de un bundle que esperaba que `/api/songs` devolviera un array). Ahora en dev ese puerto responde un cartel que te manda a `:5174`; el build solo se sirve con `pnpm start`.
